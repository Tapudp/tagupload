...
const L0TagsUrl = "https://bff.airmeet.com/api/v1/tag/?tagType=LEVEL0";
const L1TagsUrl = "https://bff.airmeet.com/api/v1/tag/?tagType=LEVEL1";
const postL1TagsUrl = "https://bff.airmeet.com/api/v1/tag";
...
const verifyL1Tags = async (existingL1Tags) => {
  ...
  existingL1Tags.forEach((e) => {
    pureNewL1Tags = pureNewL1Tags.filter(
...
const attachNewL1TagsWithL0Tags = async (
  l1Tags,
  currentlyFetchedL0Tags = []
) => {
  console.log(
  ...
  l1Tags.forEach((x, idx) => {
    currentlyFetchedL0Tags.forEach((y, yidx) => {
      x[0].replace('Area of Interest','Interest Area').replace('Industries','Industry').trim().toLowerCase() === y.name.trim().toLowerCase() &&
...
const work = async () => {
  ...
  try {
    ...
    if (!error) {
      ...
      const l1TagsReadyToSend = await attachNewL1TagsWithL0Tags(
        verifiedNewL1tags,
        data.LEVEL0
      );
      ...
      if (!success) {
        console.log(
          "there is some error while posting level-1 tags",
          errorMessage
        );
      } else {
